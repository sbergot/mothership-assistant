    ...
    <div className="flex flex-col">
      <Block variant="light">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <ClassSummary className="marine" onClick={setSelectedClass} selected={selectedClass == "marine"}>
            <div>+10 combat</div>
            <div>+10 body save</div>
            <div>+20 fear save</div>
            <div>+1 wound</div>
          </ClassSummary>
          <ClassSummary className="android" onClick={setSelectedClass} selected={selectedClass == "android"}>
            <div>+20 intellect</div>
            <div>-10 to 1 stat</div>
            <div>+60 to fear save</div>
            <div>+1 wound</div>
          </ClassSummary>
          <ClassSummary className="teamster" onClick={setSelectedClass} selected={selectedClass == "teamster"}>
            <div>+5 to all stats</div>
            <div>+10 to all saves</div>
          </ClassSummary>
          <ClassSummary className="scientist" onClick={setSelectedClass} selected={selectedClass == "scientist"}>
...
function ClassSummary({ className, selected, children, onClick }: ClassSummaryProps) {
  const classes = selected ? "bg-mother-6" : "bg-mother-1"
  return (
    <div onClick={() => onClick(className)} className="w-48 rounded-xl bg-mother-4 text-mother-1 flex flex-col gap-2 pb-2 cursor-pointer">
      <div className="rounded-3xl bg-mother-6 text-center relative">
        <div className={`circle-small ${classes} absolute left-0.5 top-0.5 border-2 border-mother-1`} />
        {className}
      </div>
